<#
.SYNOPSIS
Simulates encoded powershell being called including command line auditing, uses Windows Sysmon Event Logs.

.DESCRIPTION
Takes four arguments: -computer, -loginDomain, -loginName, -powershellCommand

.EXAMPLE
./C:\Windows\System32\WindowsPowershell\v1.0\powershell.exe -file WinSec-EncodedPowerShell.ps1 -computer "lrxm" -loginDomain "lrxm" -loginName "username" -psexecCommand "`"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe`" -ExecutionPolicy Bypass -w hidden -encodedCommand ZABpAHIAIAAiAGMAOgBcAHAAcgBvAGcAcgBhAG0AIABmAGkAbABlAHMAIgAgAA=="
...
param(
  ...
  [Parameter(Mandatory=$false)]
  [String]$powershellCommand = '`"C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe`" -ExecutionPolicy Bypass -w hidden -encodedCommand ZABpAHIAIAAiAGMAOgBcAHAAcgBvAGcAcgBhAG0AIABmAGkAbABlAHMAIgAgAA=='
  )
